...
def _validate_test_script(test_script):
    msg = ""
    if not test_script:
...
def _validate_job(job):
    ...
    msg = _validate_job_name(job_name)
    if msg:
        return msg

    bin_path = job.get("bin_path")
    msg = _validate_bin_path(bin_path)
    if msg:
        return msg

    test_script = job.get("script")
    msg = _validate_test_script(test_script)
    if msg:
        return msg

    install_script = job.get("install")
    msg = _validate_install_script(install_script)
    if msg:
        return msg
    ...
    if not jobs:
        ...
        if not isinstance(jobs, list):
            ...
            if len(jobs) > 3:
                ...
                for job in jobs:
                    msg = _validate_job(job)
                    if msg:
                        break

    run_on = config.get("run_on")
    msg = _validate_run_on(run_on)
